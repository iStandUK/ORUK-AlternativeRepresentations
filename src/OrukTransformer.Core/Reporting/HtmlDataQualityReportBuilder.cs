...
namespace OrukTransformer.Core.Reporting;

/// <summary>
/// Builds a self-contained xHTML5 VODIM data-quality report (with embedded
/// iStandUK-branded CSS) as a string. Shared by the CLI's file writer and the MCP
/// data-quality tool so both emit an identical report. All VODIM roll-up is delegated
/// to <see cref="VodimSummary"/>; this class only formats.
/// </summary>
public static class HtmlDataQualityReportBuilder
{
    /// <summary>
    /// Builds the complete xHTML5 report document for the given transformation reports.
    /// </summary>
    /// <param name="reports">Per-service VODIM reports to summarise.</param>
    /// <param name="sourceUrl">The feed the services came from (shown in the header).</param>
    /// <param name="overallWarnings">Optional input warnings to surface in the summary.</param>
        ...
        IReadOnlyList<string>? overallWarnings = null)
    {
        ...
        sb.AppendLine($"      <p><strong>Services assessed:</strong> {summary.ServiceCount}</p>");
        ...
        sb.AppendLine("          <tbody>");
        AppendVodimRow(sb, "V", "Valid", overall.Valid, overall.Total, "valid", indent: 12);
        AppendVodimRow(sb, "O", "Other", overall.Other, overall.Total, "other", indent: 12);
        AppendVodimRow(sb, "D", "Default", overall.Default, overall.Total, "default", indent: 12);
        AppendVodimRow(sb, "I", "Invalid", overall.Invalid, overall.Total, "invalid", indent: 12);
        AppendVodimRow(sb, "M", "Missing", overall.Missing, overall.Total, "missing", indent: 12);
        AppendVodimRow(sb, "U", "Unmapped", overall.Unmapped, overall.Total, "unmapped", indent: 12);
        sb.AppendLine("          </tbody>");
        sb.AppendLine("          <tfoot><tr>");
        sb.AppendLine($"            <td colspan=\"2\"><strong>Total fields</strong></td>");
        sb.AppendLine($"            <td><strong>{overall.Total}</strong></td><td></td>");
        sb.AppendLine("          </tr></tfoot>");
        sb.AppendLine("        </table>");

        // Add pie chart
        AppendPieChart(sb, overall.Valid, overall.Other, overall.Default,
            overall.Invalid, overall.Missing, overall.Unmapped, overall.Total);
        ...
        else
        {
            foreach (var field in summary.Fields)
            {
                ...
                var anchorId = "field-" + Encode(field.SourcePath
                    .Replace('.', '-').Replace('[', '-').Replace(']', '-'));

                sb.AppendLine($"      <section class=\"field\" id=\"{anchorId}\">");
                sb.AppendLine($"        <h2 class=\"field-heading\">{Encode(field.SourcePath)}</h2>");
                sb.AppendLine($"        <p class=\"target-path\">→ <code>{Encode(field.TargetPath)}</code></p>");
                ...
                sb.AppendLine("            </tr></thead>");
                sb.AppendLine("            <tbody>");
                AppendVodimRow(sb, "V", "Valid", c.Valid, c.Total, "valid", indent: 14);
                AppendVodimRow(sb, "O", "Other", c.Other, c.Total, "other", indent: 14);
                AppendVodimRow(sb, "D", "Default", c.Default, c.Total, "default", indent: 14);
                AppendVodimRow(sb, "I", "Invalid", c.Invalid, c.Total, "invalid", indent: 14);
                AppendVodimRow(sb, "M", "Missing", c.Missing, c.Total, "missing", indent: 14);
                AppendVodimRow(sb, "U", "Unmapped", c.Unmapped, c.Total, "unmapped", indent: 14);
                sb.AppendLine("            </tbody>");
                sb.AppendLine("          </table>");

                // Add field-level pie chart
                AppendPieChart(sb, c.Valid, c.Other, c.Default,
                    c.Invalid, c.Missing, c.Unmapped, c.Total, indent: 10);
                ...
                if (field.Notes.Count > 0)
                {
                    ...
                    sb.AppendLine("            <tbody>");
                    foreach (var note in field.Notes)
                    {
                        sb.AppendLine(
                            $"              <tr class=\"{ClassificationCssClass(note.Classification)}\">" +
                            $"<td>{Encode(note.Text)}</td>" +
                            $"<td class=\"count\">{note.Count}</td></tr>");
                    }
                    sb.AppendLine("            </tbody>");
                    sb.AppendLine("          </table>");
                    sb.AppendLine("        </div>");
