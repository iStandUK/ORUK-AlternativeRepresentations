...
## 11. Impact on the ORUK→Schema.org conversion

Full engineering analysis in **[nhs-dohs-schema-org-impact.md](nhs-dohs-schema-org-impact.md)** (grounded in the actual transformer source and Schema.org models, adversarially verified). Headlines:

- **NHS sourcing *inverts* the completeness profile, it doesn't uniformly enrich.** A few Place/identity fields that native ORUK feeds ship empty flip to reliably populated — `Place.identifier` (authoritative UPRN), `Place.geo` (via a new UPRN→OS Open UPRN step), and a coded `Service.additionalType`/`serviceType` from **ODS roles** (native feeds emit *no* taxonomy at all — issue #81, so NHS actually out-populates them). Meanwhile the whole Service payload (`audience`/eligibility, `offers`/cost, `availableLanguage`, `openingHoursSpecification`, assurance) stays `Missing`.
- **The `DefinedTerm` machinery already fits.** `SchemaOrgDefinedTerm` carries `@id`/`inDefinedTermSet`/`termCode`/`name`; an ODS role (code + display + CodeSystem) drops straight in. Work needed: add an `ods-role` scheme to the vocabulary registry (`KnownTaxonomyPrefixes` **and** `ConstructTaxonomyUri`, in lockstep) with a full-CodeSystem-URI `inDefinedTermSet`, and select the **most service-specific role** (for contractors that's the *primary* role; only GP Practice `RO76` is non-primary) upstream in the ingestion adapter.
- **UPRN becomes the `Place` `@id`/dedup key** so co-located services (shared UPRN) converge to one Place while keeping distinct Service and Organization nodes — each service's `provider` resolved from the ODS `RE6` operator (can differ from the premises owner).
- **Two pre-existing transformer defects surface here and should be fixed alongside** (both verified in `OrukToSchemaOrgTransformer.cs`): (1) the `Service.location` reference is emitted as `{"type":"@id","id":…}` (line 263) instead of standard JSON-LD `{"@id":…}` — unlike `provider`/`serviceOperator`; (2) the `OrukLocation.Uprn` scalar is **never emitted** (only `external_identifiers[UPRN]` reaches output), yet VODIM scores `location.uprn` as `Valid` when the scalar is present — over-reporting UPRN coverage.
- **VODIM will read as a mass failure unless taught** to distinguish source-limited `Missing` (eligibility/cost/languages — not sourceable from NHS) from publisher-omission, to count skipped-defunct records, and to mark geocode precision/provenance.